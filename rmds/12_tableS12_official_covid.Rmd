---
title: "Table S12. In-hospital and out-of-hospital COVID-19 and all-cause deaths by characteristic"
output: 
  html_document:
    toc: true
    toc_float: true
...
summarize_deaths <- function(deaths_df) {
    covid_df <- deaths_df |>
        filter(placdth_str == "hospital_inpatient") |>
        summarize(
            n_covid_inhosp = sum(has_covid),
            n_allcause_inhosp = n()
        ) |>
        ungroup() |>
        mutate(prop_inhosp_covid = n_covid_inhosp / n_allcause_inhosp)
    all_cause_df <- deaths_df |>
        filter(placdth_str != "hospital_inpatient") |> 
        summarize(
            n_covid_outhosp = sum(has_covid),
            n_allcause_outhosp = n()
        ) |>
        ungroup() |>
        mutate(prop_outhosp_covid = n_covid_outhosp / n_allcause_outhosp)
...
bind_rows(total_df,
          ...
          diabetes_df) |> 
    mutate(n_covid_inhosp = prettyNum(n_covid_inhosp, big.mark = ","),
           n_allcause_inhosp = prettyNum(n_allcause_inhosp, big.mark = ","),
           prop_inhosp_covid = sprintf("%0.3f", round(prop_inhosp_covid, 3)),
           n_covid_outhosp = prettyNum(n_covid_outhosp, big.mark = ","),
           n_allcause_outhosp = prettyNum(n_allcause_outhosp, big.mark = ","),
           prop_outhosp_covid = sprintf("%0.3f", round(prop_outhosp_covid, 3))) |> 
    kable(
        format = "html", 
        escape = FALSE
    ) |> 
