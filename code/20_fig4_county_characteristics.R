...
county_df <- results_df |>
    ...
    dplyr::filter(grouping_var %in% GROUPING_VARS, !is.na(grouping_value)) |>
    calculate_arr()
...
p1 <- ggplot2::ggplot(
    ...
    ggplot2::scale_x_continuous("Adjusted Reporting Ratio (95% UI)") +
    mk_nytimes(axis.text.y = ggplot2::element_text(hjust = 0, face = y_face))

## Save ----
ggplot2::ggsave(
    here::here("plots", "fig4_county.pdf"),
    p1,
    width = 6,
    height = 7,
    scale = 1,
...
ggplot2::ggsave(
    here::here("plots", "fig4_county.jpg"),
    p1,
    width = 6,
    height = 7,
    scale = 1,
...
readr::write_csv(
    ...
    here::here("output", "figure_data", "fig4_data_county.csv")
)
