## 16_plot_model_selection_main.R ----
## Imports ----
library(here)
library(tidyverse)
...
sub_metrics <- sub_metrics |>
    group_by(metric, estimator, metric_cat) |>
    mutate(pomp = mean / max(mean) * 100, .before = 4) |>
    ungroup()

## Figure ----
p1 <- ggplot2::ggplot(sub_metrics,
                      ggplot2::aes(x = pomp, y = model_pre_cat_rev, color = model_cat, )) +
    ggplot2::annotate(
        "rect",
        xmin = rep(-Inf, 2),
        xmax = rep(Inf, 2),
    ...
    ggplot2::scale_x_continuous("Percent of maximum possible value (%)") +
    ggplot2::scale_color_brewer("Model type", palette = "Dark2") +
    mk_nytimes(legend.position = "none")

## Save ----
ggplot2::ggsave(
    here::here("plots", "figS10_pomp.pdf"),
    p1,
    width = 10,
    height = 4,
...
ggplot2::ggsave(
    here::here("plots", "figS10_pomp.jpg"),
    p1,
    width = 10,
    height = 4,
    dpi = 1200
