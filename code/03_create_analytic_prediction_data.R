## 03_create_analytic_prediction_data.R ----
##
## This file takes the out-of-hospital prediction data set and incorporates
...
## Data ----
### Read in raw data ----
raw_data <- readRDS(here::here("data_private", "raw_data_prediction_only.RDS"))
N_IMPORT <- NROW(raw_data)

### Read in aux data ----
county_df <- readr::read_csv(here::here(
    "data_raw",
    "County_Covariates_Sociodemographics_2021.csv"
))
rucc_df <- readxl::read_excel(here::here(
    "data_raw",
...
vacc_df <- readr::read_csv(here::here(
    "data_raw",
    paste0(
        "COVID-19_Vaccinations_in_the_",
        "United_States_County.csv.zip"
    )
))
...
working_df <- raw_data |>
    ...
    dplyr::rename(has_covid_contributing = has_covid) |>
    flag_covid_death(ucod_only = TRUE) |>
    dplyr::rename(has_covid_underlying = has_covid) |>
    dplyr::mutate(has_covid = has_covid_contributing + has_covid_underlying) |>
    dplyr::select(-has_covid_contributing, -has_covid_underlying) |>
    flag_copds() |>
    flag_hyperlipidemia() |>
    flag_hypertensives() |>
    flag_ischemic() |>
...
sub_vac <- vacc_df |>
    ...
    dplyr::select(date,
        ...
        month,
        county_fips = fips,
        tidyr::matches("\\<series_complete_.+pct\\>")
    ) |>
...
analytic_df <- working_df |>
    dplyr::mutate(
        ...
        comm_trans_level_str =
            ifelse(is.na(community_transmission_level),
                "unknown",
                community_transmission_level
            ),
...
## Save ----
saveRDS_xz(analytic_df, here::here("data_private", "analytic_prediction_data.RDS"))

## Take some notes about sample size ----
sink(here::here("output", "sample_size_notes.txt"), append = TRUE)
cat("-----------------   ")
cat(sprintf("Creation of out-of-hospital (prediction) analytic data"))
cat("   -----------------\n")
cat(sprintf("Run on: %s\n", Sys.time()))
cat(sprintf("Deaths in prediction data at import: %i\n", N_IMPORT))
cat(sprintf("Deaths in prediction data after removing under 25 y/o: %i\n", N_AGE))
cat(sprintf(
    "Deaths in prediction data after removing external causes of death: %i\n\n",
    N_EXTERNAL
))
sink()
